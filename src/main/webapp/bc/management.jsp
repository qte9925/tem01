...
<div id="myTabContent" class="tab-content">
    <div class="tab-pane fade in active" id="home">
        <div style="width: 700px;height: 30px;border: 1px solid white;">考核名称<input type="text" id = "tname" placeholder="考核名称"/>
            <input type="button" id = "Btn" value = "查询" class="btn btn-default"  style="margin-top: -10px"/></div>
        <table border="1px" class="table table-hover" id="customers" style="text-align: center;">
            ...
            <tr v-for="post in plist">
                <td style="display: none" class = "Tid">{{post.tb_task_id}}</td>
                <td>{{post.tb_task_name}}</td>
                <td class = "DetailBtnOne" id ="{{post.tb_task_id}}"><type data-toggle="modal" data-target="#Nameid "/><span style="cursor: pointer;"/>{{post.tb_staff_name}}</td>
                <td class = "DetailBtnTwo" id ="{{post.tb_task_name}}"><type data-toggle="modal" data-target="#NameidTwo"/><span style="cursor: pointer;"/>{{post.tb_prople_ygkh_name}}</td>
                <td>{{post.tb_task_startTime}}</td>
                <td>{{post.tb_task_endTime }}</td>
                <td style="display: none"><input type = "hidden" class = "Etime" value = "{{post.tb_task_endTime }}"/></td>
                <td class = "DetailBtnThree" id ="{{post.tb_task_id}}"><type data-toggle="modal" data-target="#NameidThree"/><span style="cursor: pointer;"/>考核项明细</td>
                <td v-if = "post.tb_task_state == 1"  calss = "taskstate">失效</td>
        ...
        <div id="Page">
            <button id="upPage" class="btn btn-info">上一页</button>&nbsp;
            第<span id="pageNow"></span>页  /  共<span id="allPage"></span>页&nbsp;
            <button id="downPage" class="btn btn-info">下一页</button>&nbsp;&nbsp;
        </div>
    </div>
